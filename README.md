...
| 子系统 | 功能 | 关键规格 | 详设 |
|---|---|---|---|
| 前端 | 分支预测 → 取指 → 预译码 → 译码 | 8 KB 直映 ICache；FQ 4 / IQ 4；TAGE 方向 + BTB/Target Cache + SARAS | [`frontend.md`](docs/frontend.md) |
...
- **TAGE-SC**：统计校正器曾试装后移除（简化版全线退化），待按 Seznec 论文补充分历史长度计数器后重试。
