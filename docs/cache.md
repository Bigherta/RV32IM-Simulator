...
> 的指令/数据主存（IMEM / DMEM）。**主存延迟固定 50 周期**，L1 命中零延迟
> （ICache 当拍组包、DCache 命中 1 拍自答）；两个缓存缺失回填与脏逐出写回统一
> 走 50 周期延迟。访存队列如何驱动它们见 [访存子系统](memory.md)。
> [← 返回 README](../README.md)
...
| 层次 | 容量/组织 | 命中 | 缺失 |
|------|-----------|------|------|
| **L1I** · ICache | 8 KB，512 行 × 16 B，**直接映射** | 当拍组包入 FQ（零命中延迟） | IMEM 整行回填，**50 周期** |
| **L1D** · DCache | 64 KB，1024 组 × 4 路 × 16 B，**tree-PLRU** | load **1 拍自答**；store 当场落行 | 主存回填 **50 周期**；脏 victim 并行写回 **50 周期** |
| **IMEM / DMEM** | 各 128 KB 字节寻址 | — | — |

主存延迟是唯一的外部延迟常量：取指侧 `IMEM.pushRequest` 与数据侧
`DCache` 缺失/写回请求都登记 `remain_cycle = 50`（注释 `main-memory latency
(benchmarks.md)`，与 [docs/benchmarks.md](benchmarks.md) 口径一致）。
...
- 行级突发服务：请求队列 + `remain_cycle=50` 递减；返回经 **`LineReturn` 四字
  总线**（`{lineAddr, data[0..3]}`，word 索引 = `(pc>>2)&3`）交给 ICache；
- 与 DCache 回填共享同一主存延迟口径（50 周期）。
...
- 编译期参数可覆写：`-DNUM_OF_SETS=64 -DDCACHE_INDEX_BITS=6` 缩小容量以强制
  容量逐出、压测脏写回路径（`DCACHE_TAG_SHIFT = 4 + INDEX_BITS`，有
  `static_assert` 守护几何一致性）。
...
READY ── 命中 ───────────────────────────────► load: 1 拍自答 loadResp / store: 当场写行置脏
  ...
  └── 缺失 ── latch 身份( park ) + busy=1 ──► WAIT
       （向 DMEM 双通道发：回填读[+ victim 脏时写回写]，各 50 周期）
...
- **双通道脉冲**：向 DMEM 发 `DMEMRequest{readValid, writeValid}`。victim 行脏
  时读、写**并行**发出（读回填 + 写回脏行），各 50 周期；**写回地址由 victim
  行自身的 tag 重建**（而不是请求地址的 tag——否则脏数据会落到错误帧，回填再
  读回陈旧值）；
- **WAIT 完成**：`DMEM.isReplyReady() ∧ ¬isWriteBusy()` 时，把 16 B 行 memcpy 进
  `targetWay`（清 dirty、写 tag、置 valid、更新 PLRU），然后服务 park：load →
  提取字节 + 符号扩展填入 `loadBuffer`；store → 逐字节落行并置脏。最后清
  park/request/busy 回 `READY`；
- **行阵不进快照**：`cacheSets` 是堆上属主实例独有的大状态，`snapshotFrom` 只
  复制 `busy/phase/request/loadBuffer/cacheRequestBuffer`——reorder 一致性校验
  针对行阵另做整阵指纹（`reorder_test` 的 `dcacheDigest`，见 README §6.2）。

## 4. DMEM（双口主存）

- **读/写双口各自独立**：`readBusy`/`writeBusy`、`readExecute`/`writeExecute`
  分开计数递减，因此回填读与脏逐出写回可**并行在飞**（各 50 周期）；
- 只接受 DCache 转发的**行级请求**（读行/写行），不再接受任何处理器侧的
  Load/Store 原语；
- 完成读时从**活体** `CPUstate.DMEMModule.mem` 取 16 B 填 `readOutputBuffer`
  （回填必须能看到此前脏逐出写回的新值——存储阵列不进快照，同 IMEM 约定）；
  写回经 `writeLine` 落活体阵列；
- `MemPull` 消费已就绪的回复（`readBufferValid` 清零）。

## 5. 专项验证与画像

- **脏逐出微测试**：构造同组不同 tag 的 store 强制逐出 ×2（写回 50 周期路径），
  末尾 lw 读回验证脏数据落 DMEM 且回填正确；
- **命中率画像**：18 基准的 I$ / D$ 命中率与逐分型预测准确率在
  [`benchmarks.md`](benchmarks.md)（D$ 覆盖 load+store 全部访存）；
...
| 主存延迟 | 固定 **50 周期**（IMEM 回填、DCache 回填、脏逐出写回共用） |
| DMEM | 读/写双口独立在飞（各 50 周期）；仅行级请求 |
| 存储阵列 | IMEM/DMEM 各 128 KB；阵列不进快照，属活体实例 |
| 一致性复核 | reorder_test 对 DMEM 镜像 FNV 与 DCache 行阵 digest 做指纹比对 |
...
- [`benchmarks.md`](benchmarks.md) — 主存延迟口径与命中率实测
